(() => {
  ...
  function registerProduct(product){if(!product?.id)return;catalog[product.id]={...product};}
  window.DESHIGRAM_CART={getCart,saveCart,addProduct:add,registerProduct,updateQuantity:update,removeProduct:remove,clearCart:clear,getSubtotal:subtotal,getMrpTotal:mrpTotal,getItemCount:count,openDrawer,closeDrawer,catalog,money};document.addEventListener("DOMContentLoaded",()=>{injectCartUI();renderCart();});
})();
